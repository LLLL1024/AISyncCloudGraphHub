package com.xiyan.xipicture.shared.auth;
...
import static com.xiyan.xipicture.domain.user.constant.UserConstant.USER_LOGIN_STATE;
...
public class StpInterfaceImpl implements StpInterface {
    ...
    // todo 11.2 权限校验还有可以用其他的方法实现：直接封装权限校验方法，在业务代码中调用。
    // todo 11.2 可以给空间操作（SpaceController）、空间分析操作（SpaceAnalyzeController）增加统一的权限校验，都可以直接加个管理员的权限。

    /**
     * 返回一个账号所拥有的权限码集合
     */
    //  注意，采用注解式鉴权 + 通过请求对象获取参数时，可能会重复查询数据库。比如业务代码中已经有根据 id 查询空间信息的代码了，但为了权限校验，也查库获取了一次空间信息，会对性能造成影响
    @Override
    ...
    private SpaceUserAuthContext getAuthContextByRequest() {
        ...
        if (ContentType.JSON.getValue().equals(contentType)) {
            // 使用 Hutool 的工具类 ServletUtil 从 HttpServletRequest 中获取到了参数信息。
